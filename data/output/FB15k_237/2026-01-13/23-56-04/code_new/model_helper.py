...
def train_step(model, data, kg, optimizer, scheduler):
    ...
    # Use a subgraph with edges removed (non-inplace!)
    if cfg.rm_rate > 0:
        # Create edge mask: True for edges to KEEP
        edge_mask = torch.ones(kg.num_edges(), dtype=torch.bool, device=device)
        edge_mask[rm_edges] = False
        kg_train = dgl.edge_subgraph(kg, edge_mask, relabel_nodes=False)
    else:
        kg_train = kg

    score, corr = model(src, rel, kg_train)
    loss = model.loss(score, label, corr)
...
def evaluate(model, set_flag, kg, record=False) -> dict:
    ...
    with torch.no_grad():
        ent_emb, rel_emb, corr = model.aggragate_emb(kg)
